# -*- coding: utf-8 -*-
"""코스피 정식 명단 생성 (KRX/FinanceDataReader 기준, 권위 있는 현재 상장 목록).

기존 corp_index(CORPCODE.xml, 상폐 포함 느슨한 목록) 대신,
FinanceDataReader('KOSPI')의 현재 상장 종목에서 우선주를 제외하고
DART corp_code로 매핑되는 '고유 회사'만 추려 kospi_index.json에 저장한다.
시가총액 큰 순 정렬(배치 수집 시 대형주 우선).

- 상폐 섞임 없음(현재 상장만), DART 전수 태깅 불필요.
- 주가/거래대금/시총은 FinanceDataReader가 함께 제공(수집·스크리닝 단계에서 활용).

사용: py screener/build_kospi_universe.py
"""
...
def main():
    ...
    print(f"FDR 코스피 종목(우선주 포함): {len(df)}")
    ...
    KOSPI_INDEX.write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"코스피 정식 명단: {len(rows)}개 회사 → {KOSPI_INDEX.name} (시총 큰 순)")
